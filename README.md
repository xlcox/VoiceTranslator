...
3. **Логирование (core/logger_config.py)** - Единая система логирования с выводом в файл (logs/app.log) и консоль. Форматирование включает временные метки, уровни, имена модулей. Поддерживает UTF-8.

4. **SoundPad Manager (core/soundpad_manager.py)** - Управление подключением и воспроизведением через SoundPad с использованием библиотеки soundpad_control. Поддерживает автозапуск SoundPad, обнаружение запущенного процесса, воспроизведение в динамики и микрофон одновременно, очистку добавленных звуков после воспроизведения, повторные попытки при ошибках.

5. **Голосовой переводчик (core/voice_translator.py)** - Основной модуль обработки. Содержит:
...
- **Асинхронная архитектура**: Основной цикл использует asyncio для неблокирующей обработки, с отдельными пулами потоков для тяжелых операций (Whisper, перевод).
- **Офлайн-перевод**: Используется Argos Translate с локальными моделями, не требует подключения к интернету для перевода.
- **Отказоустойчивость**: Многоуровневая обработка исключений, повторные попытки подключения к SoundPad, проверка состояний.
- **Гибкая конфигурация**: Все параметры настраиваются через JSON-файл, включая пути, таймауты, параметры аудио и голоса.
- **Управление состоянием SoundPad**: Автозапуск SoundPad при необходимости, проверка подключения, принудительная остановка воспроизведения.

### Конфигурация

Приложение использует JSON-конфигурацию с секциями:
- **app**: уровень логирования (log_level) и горячая клавиша (hotkey)
- **audio**: параметры аудиозаписи (частота дискретизации, минимальная длительность, временный файл)
- **translation**: языки перевода (source_lang, target_lang), модель Whisper, движок перевода (argos)
- **tts**: параметры синтеза речи (голос, скорость, громкость)
- **soundpad**: настройки SoundPad (включен, автозапуск, путь, каналы воспроизведения, задержки, повторные попытки)
